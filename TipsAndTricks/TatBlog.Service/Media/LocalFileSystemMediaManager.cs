...
public class LocalFileSystemMediaManager : IMediaManager
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var retunedFilePath = CreateFilePath(
                fileExt, contentType.ToLower());
            var fullPath = Path.GetFullPath(
                Path.Combine(Environment.CurrentDirectory,
                "wwwroot", retunedFilePath));
            ...
            return retunedFilePath;
        }
        catch (Exception ex)
        {

            _logger.LogError(ex, $"Khong the luu File '{originalFileName}'.");
            return null;
        }
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogError(ex, $"Khong the xoa file '{filePath}'.");
